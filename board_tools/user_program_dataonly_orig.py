...
class UserProgram:
    ...
    def connect(self):
        while True:
            ...
            self.board.control_connection.close() #drop control connection for data-only version.
    ...
    def monitor_main(self):
        ...
        #     resp = None
        if False and resp is not None and hasattr(resp, "configurations"):
            ...
            if hasattr(self.last_ins_msg, "raw"):
                ...
                if self.last_ins_msg.raw == last_last_ins:
                    ...
                    if active_tab == "map-tab":
                        ...
                        roll_value = ins_msg.roll_deg if hasattr(ins_msg, "roll_deg") else 0  # 0 default ok?
                        ...
                        pitch_value = ins_msg.pitch_deg if hasattr(ins_msg, "pitch_deg") else 0 #0 default ok?
    ...
    def upgrade(self):
        ...
        hex_file_path = pick_one_file(initialdir=None, title="Select hex file to load: IMU-A1_<version>.hex")
        ...
        #check filename here? then can call with expected version instead of extracting it inside.
        name_pattern = 'IMU-A1_v\\d+.\\d+.\\d+.hex' # IMU-A1_va.b.c.hex where a,b,c are one or more digits.
        if re.fullmatch(name_pattern, hex_file_name):
            sw_version = hex_file_name.replace("IMU-A1_v", "").replace(".hex", "")  # assuming "IMU-A1_va.b.c.hex"
        else:
            print(f"\nWarning: unexpected name for firmware file:    {hex_file_name}\n"
                    f"Expected name:                                 IMU-A1_va.b.c.hex (where a, b, c are the version numbers)")
            #give choice to update anyway - probably remove this in public version, keep in internal version.
            print("\nUpdate anyway?") #for customers, show and pause, then return here.
            sw_version = "unknown"

        print(f"\nThis will upgrade to version {sw_version}. Upgrade now?")
        options = ["Yes", "No"]
        if options[cutie.select(options)] == "Yes":
            ...
            try:
                self.board.bootload_with_file_path(hex_file_path, sw_version, num_attempts=3)
            except Exception as e:
            ...
            show_and_pause(f"\n\nFinished updating to {sw_version}")
